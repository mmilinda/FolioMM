...
const impactDomains = [
  {
    ...
    problem: "Accès à l'information agricole & traçabilité de la chaîne alimentaire",
    color: "#34d399",
  ...
  },
  {
    ...
    icon: Fingerprint,
    project: "AfriAccess",
    problem: "Processus d'identification numérique sécurisé & authentification",
    color: "#38bdf8",
  ...
  },
  {
    ...
    icon: Car,
    project: "Garabi",
    problem: "Accès rapide et digitalisé aux services d'assistance automobile",
    color: "#f472b6",
  ...
  },
  {
    ...
    icon: Building2,
    project: "Noregis",
    problem: "Digitalisation des processus métiers & automatisation intégrée",
    color: "#fb923c",
      ...
      <div className="container-custom relative z-10 space-y-14 md:space-y-24">
        ...
        >
          ...
          <div className="lg:col-span-7 space-y-5 text-left flex flex-col justify-between">
            <div className="space-y-4">
              <span className="section-eyebrow">
                <Target size={15} className="inline-block mr-1.5 -mt-0.5 text-cyan-400" />
                Vision & Philosophie
              </span>
              ...
              <div className="p-5 sm:p-6 rounded-2xl bg-white/5 border border-cyan-400/30 backdrop-blur-md relative overflow-hidden space-y-3 shadow-lg">
                <div className="absolute top-0 left-0 w-1.5 h-full bg-gradient-to-b from-cyan-400 via-indigo-400 to-purple-500" />
                <h3 className="text-base sm:text-xl font-bold text-white flex items-center gap-2.5 m-0">
                  <Lightbulb size={22} className="text-cyan-400 shrink-0" />
                  Technologie avec un objectif
                </h3>
                <p className="text-slate-200 text-sm sm:text-base md:text-lg leading-relaxed m-0 font-medium">
                  Je conçois des solutions numériques pour répondre à des problèmes concrets dans mon environnement.
                </p>
              </div>

              <p className="text-slate-300 text-sm sm:text-base md:text-lg leading-relaxed md:leading-loose">
            ...
            {/* Quote Block */}
            <div className="p-4 sm:p-5 rounded-2xl bg-cyan-950/40 border border-cyan-400/30 text-sm sm:text-base text-cyan-100 font-medium leading-relaxed italic shadow-md">
          ...
          <div className="lg:col-span-5 flex">
            <motion.div
              ...
              className="p-5 sm:p-8 rounded-2xl md:rounded-3xl bg-slate-900/90 border border-white/15 backdrop-blur-xl shadow-2xl flex flex-col justify-between space-y-6 text-left w-full"
            >
              <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {initiatives.map((item) => (
                    <span
                      key={item}
                      className="text-xs sm:text-sm px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-slate-200 font-medium hover:border-cyan-400/40 hover:text-cyan-300 transition-colors"
                    >
                      🚀 {item}
                    </span>
                  ))}
              ...
              <div className="pt-5 border-t border-white/10 space-y-2">
                <span className="text-xs uppercase tracking-wider font-semibold text-cyan-400 flex items-center gap-2">
                  <Compass size={16} /> Mon Objectif Ultime
                </span>
                <p className="text-sm sm:text-base text-slate-100 leading-relaxed font-semibold m-0">
        ...
        >
          <div className="text-center space-y-3">
            ...
            <p className="text-slate-300 text-sm sm:text-base md:text-lg max-w-xl mx-auto m-0 leading-relaxed">
              Chaque projet répond à un défi concret identifié sur le terrain.
            </p>
          </div>

          {/* Desktop Table View */}
          <div className="hidden md:block overflow-hidden rounded-2xl border border-white/15 bg-slate-900/80 backdrop-blur-md shadow-2xl">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/10 bg-white/5 text-slate-200 text-sm font-bold uppercase tracking-wider">
                  ...
                  return (
                    <tr key={item.domain} className="hover:bg-white/5 transition-colors">
                      <td className="py-4 px-6 font-semibold">
                        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs font-bold" style={{ background: item.bg, borderColor: item.border, color: item.color }}>
                          <Icon size={16} />
                          {item.domain}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-6 text-slate-300 font-normal leading-relaxed">
                        {item.problem}
                      </td>
                    </tr>
                  );
          ...
          {/* Mobile Cards View — Generous padding & spaced typography */}
          <div className="grid grid-cols-1 gap-4 md:hidden">
            {impactDomains.map((item) => {
              ...
              return (
                <div
                  key={item.domain}
                  className="p-4 sm:p-5 rounded-2xl border bg-slate-900/80 backdrop-blur-md flex flex-col space-y-3 text-left shadow-lg"
                  style={{ borderColor: item.border }}
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold" style={{ background: item.bg, color: item.color }}>
                      <Icon size={14} />
                      {item.domain}
                    </span>
                    <span className="text-sm font-extrabold text-white tracking-wide">{item.project}</span>
                  </div>
