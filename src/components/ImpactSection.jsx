...
export default function ImpactSection() {
  ...
  return (
    <section ref={ref} className="py-14 md:py-32 relative overflow-hidden">
      <div className="container-custom relative z-10 space-y-16 md:space-y-32">
        ...
        >
          ...
          <div className="lg:col-span-7 space-y-6 text-left flex flex-col justify-between">
            <div className="space-y-6">
              ...
              <div className="p-6 sm:p-9 rounded-2xl md:rounded-3xl bg-slate-900/90 border-2 border-cyan-400/40 backdrop-blur-xl relative overflow-hidden space-y-4 shadow-2xl">
                <div className="absolute top-0 left-0 w-2.5 h-full bg-gradient-to-b from-cyan-400 via-indigo-400 to-purple-500" />
                <h3 className="text-lg sm:text-2xl font-bold text-white flex items-center gap-3 m-0">
                  <Lightbulb size={26} className="text-cyan-400 shrink-0" />
                  Technologie avec un objectif
                </h3>
                <p className="text-slate-100 text-base sm:text-xl md:text-2xl leading-relaxed m-0 font-medium">
                  Je conçois des solutions numériques pour répondre à des problèmes concrets dans mon environnement.
                </p>
              </div>

              <p className="text-slate-200 text-base sm:text-lg md:text-xl leading-relaxed md:leading-loose">
            ...
            {/* Quote Block */}
            <div className="p-6 sm:p-8 rounded-2xl bg-cyan-950/60 border border-cyan-400/40 text-base sm:text-xl text-cyan-100 font-medium leading-relaxed italic shadow-2xl mt-4">
          ...
          <div className="lg:col-span-5 flex">
            <motion.div
              ...
              className="p-7 sm:p-10 rounded-2xl md:rounded-3xl bg-slate-900/90 border border-white/20 backdrop-blur-xl shadow-2xl flex flex-col justify-between space-y-10 text-left w-full"
            >
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <span className="text-sm text-slate-400">Initiatives & Leadership</span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  {initiatives.map((item) => (
                    <span
                      key={item}
                      className="text-xs sm:text-base px-4 py-2 rounded-full bg-white/5 border border-white/15 text-slate-200 font-medium hover:border-cyan-400/50 hover:text-cyan-300 transition-all shadow-sm"
                    >
                      🚀 {item}
                    </span>
                  ))}
              ...
              <div className="pt-6 border-t border-white/15 space-y-3">
                <span className="text-xs sm:text-sm uppercase tracking-widest font-extrabold text-cyan-400 flex items-center gap-2">
                  <Compass size={18} /> Mon Objectif Ultime
                </span>
                <p className="text-base sm:text-lg md:text-xl text-slate-100 leading-relaxed font-bold m-0">
        ...
        >
          <div className="text-center space-y-4">
            ...
            <p className="text-slate-300 text-base sm:text-xl md:text-2xl max-w-2xl mx-auto m-0 leading-relaxed">
              Chaque projet répond à un défi concret identifié sur le terrain.
            </p>
          </div>

          {/* Desktop Table View — Ultra spacious padded rows */}
          <div className="hidden md:block overflow-x-auto rounded-3xl border border-white/20 bg-slate-900/90 backdrop-blur-xl shadow-2xl">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/15 bg-white/5 text-cyan-400 text-sm md:text-base font-extrabold uppercase tracking-widest">
                  <th className="py-6 px-8">Domaine</th>
                  <th className="py-6 px-8">Projet</th>
                  <th className="py-6 px-8">Problème Résolu</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/10 text-base md:text-xl">
                {impactDomains.map((item) => {
                  const Icon = item.icon;
                  return (
                    <tr key={item.domain} className="hover:bg-white/5 transition-colors">
                      <td className="py-7 px-8 font-semibold">
                        <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full border text-sm md:text-base font-bold shadow-sm" style={{ background: item.bg, borderColor: item.border, color: item.color }}>
                          <Icon size={18} />
                          {item.domain}
                        </div>
                      </td>
                      <td className="py-7 px-8 font-black text-white text-lg md:text-xl">
                        {item.project}
                      </td>
                      <td className="py-7 px-8 text-slate-200 font-normal leading-relaxed md:leading-loose">
                        {item.problem}
                      </td>
                    </tr>
                  );
          ...
          {/* Mobile Cards View — Ultra spacious, readable cards */}
          <div className="grid grid-cols-1 gap-6 md:hidden">
            {impactDomains.map((item) => {
              ...
              return (
                <div
                  key={item.domain}
                  className="p-6 sm:p-8 rounded-3xl border-2 bg-slate-900/95 backdrop-blur-md flex flex-col space-y-4 text-left shadow-2xl"
                  style={{ borderColor: item.border }}
                >
                  <div className="flex items-center justify-between gap-4">
                    <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs sm:text-sm font-bold" style={{ background: item.bg, color: item.color }}>
                      <Icon size={16} />
                      {item.domain}
                    </span>
                    <span className="text-base sm:text-xl font-black text-white tracking-wide">{item.project}</span>
                  </div>
                  <p className="text-sm sm:text-base text-slate-100 leading-relaxed m-0 font-medium">
                    {item.problem}
                  </p>
                </div>
              );
